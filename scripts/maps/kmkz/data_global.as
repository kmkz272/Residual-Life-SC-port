...
class CDataGlobal : ScriptBaseEntity
{
	...
	void TriggerUse(CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float flValue)
	{
		...
		{
			DataGlobalReadFromFileToArray("scripts/maps/store/dataglobal_"+ map_series + ".txt");
			int gi = atoi(global_name.SubString(global_name.Length()-4,global_name.Length()-1));						// d is the var part of the netname keyvalue. In global017 d= 17.
		...
		if (useType == USE_OFF)
		{
			global_state = 0;
			self.pev.health = 0;
			int gi = atoi(global_name.SubString(global_name.Length()-4,global_name.Length()-1));
...
void DataGlobalWriteLineToFile(string filename)
{
	...
	try 
	{
		...
		if(file !is null && file.IsOpen())
		{
			...
			{
				g_Game.AlertMessage( at_console, "a" );
				string sLine;
				file.ReadLine(sLine);
